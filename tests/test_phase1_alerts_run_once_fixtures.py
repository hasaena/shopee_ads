...
FIXTURES_OPEN = REPO_ROOT / "tests" / "fixtures" / "shopee_ads_alerts" / "open"
FIXTURES_RESOLVED = REPO_ROOT / "tests" / "fixtures" / "shopee_ads_alerts" / "resolved"
...
def test_phase1_alerts_run_once_fixtures_open_cooldown_resolve(tmp_path: Path) -> None:
    ...
    incidents_1 = _count_rows(db_path, "SELECT COUNT(*) FROM ads_incident")
    assert incidents_1 >= 1

    # Health fixtures should not open pacing incidents.
    pacing_1 = _count_rows(
        db_path, "SELECT COUNT(*) FROM ads_incident WHERE incident_type LIKE 'pacing_%'"
    )
    assert pacing_1 == 0
    ...
    start_logs = _count_rows(
        db_path, "SELECT COUNT(*) FROM event_log WHERE message = ?", ("phase1_alerts_run_once_start",)
    )
    end_logs = _count_rows(
        db_path, "SELECT COUNT(*) FROM event_log WHERE message = ?", ("phase1_alerts_run_once_end",)
    )
    assert start_logs >= 1
    assert end_logs >= 1
    ...
    resolved = _count_rows(db_path, "SELECT COUNT(*) FROM ads_incident WHERE status = 'RESOLVED'")
    assert resolved >= 1
